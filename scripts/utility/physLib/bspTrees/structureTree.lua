...
function PhysLib.BspTrees.StructureTree:CircleCollider(pos, radius)
    local results = {}
    self:CircleCollisionsOnBranch(pos, radius, PhysLib.LinksTree, results)
    return results
end

function PhysLib.BspTrees.StructureTree:CircleCollisionsOnBranch(position, radius, branch, results)
    if not branch then return end
    ...
    if branch.deepest then
        ...
        for i = 1, #links do
            ...
            self:CircleCollisionOnLink(position, radius, link.nodeA, link.nodeB, link, results)
        end
    ...
    for i = 1, #children do
        ...
        if x > minX - radius and x < maxX + radius and y > minY - radius and y < maxY + radius then
            self:CircleCollisionsOnBranch(position, radius, child, results)
